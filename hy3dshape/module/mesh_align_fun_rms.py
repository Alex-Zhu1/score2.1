...
def icp(source_mesh,
    ...
    for cube in tqdm(cubes, total=len(cubes), ascii=True, disable=True):
        ...
        for iter in tqdm(range(n_iter), ascii=True, total=n_iter, leave=False, disable=True):
            ...
            # 将原来的固定外点部分替换为以下更鲁棒的版本（大约改5行）
            if n_outliers > 0:
                # 改为基于距离分位数动态截断（推荐70~80%分位）
                threshold = np.percentile(dist, 75)        # 保留距离最小的75%
                inlier_mask = dist < threshold
                cost = dist[inlier_mask].mean() if inlier_mask.sum() > 10 else np.inf
                p_inlier = p[inlier_mask]
                q_inlier = q[inlier_mask]
            else:
                p_inlier = p
                q_inlier = q
                cost = dist.mean()
...
def align_meshes_rms(source_mesh_path=None, target_mesh_path=None, source_mesh=None,
                 ...
                 skip_coarse=True, fixed_coarse_scale=False,
                 iterations_coarse=150, count_source_coarse=10_000, count_target_coarse=10_000,
                 iterations_fine=100, count_source_fine=10_000, count_target_fine=20_000,
    ...
    target_mesh = tm.load(target_mesh_path, process=False, skip_materials=True, merge_primitives=True)
    ...
    transform_fine, _ = icp(source_mesh, target_mesh, 
                            ...
                            fixed_scale=fixed_coarse_scale,
                            outliers=outliers, on_surface=on_surface, 
                            min_scale=0.95, max_scale=1.05, plot=plot)
